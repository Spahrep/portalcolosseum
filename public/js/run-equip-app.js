...
async function loadData() {
  ...
  weaponsMap = {};
  weapons.forEach(w => { weaponsMap[w.name] = w; });
  consumablesMap = {};
  consumables.forEach(c => { consumablesMap[c.name || c.consumable_template?.name] = c; });
  ...
  loadout[3] = ownedConsumables[0] ? (ownedConsumables[0].name || ownedConsumables[0].consumable_template?.name) : null;
  loadout[4] = ownedConsumables[1] ? (ownedConsumables[1].name || ownedConsumables[1].consumable_template?.name) : null;

  // Backpack = remaining owned items not in loadout
  backpack = [];
  const used = new Set(loadout.filter(Boolean));
  weapons.forEach(w => { if (!used.has(w.name)) backpack.push(w.name); });
  consumables.forEach(c => {
    const nm = c.name || c.consumable_template?.name;
...
function showInspectPopup(itemName, targetEl) {
  ...
  if (weaponsMap[itemName]) {
    ...
    html += `<div class="stat-line">${c.consumable_template?.description || c.description || 'Effect'}</div>`;
  }
...
function renderLoadout() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (loadout[i]) {
      ...
      else if (consumablesMap[name]) stats = consumablesMap[name].consumable_template?.description || 'Effect';
...
async function enterPortal() {
  ...
  const payload = {
    ...
    hand_l_weapon_id: null, // would map names to ids in real impl
    hand_r_weapon_id: null,
    belt_weapon_id: null,
    consume_a: null,
    consume_b: null
  };
  // For demo, we skip full id mapping; assume server accepts names or use first ids
  // Real impl would lookup ids from maps
  try {
    ...
    bottom.innerHTML = `<div class="message-locked">ENTRY FAILED: ${e.message}</div>`;
  }
}
...
function setupEnterButton() {
  ...
  const bottom = document.getElementById('bottom-bar');
  btn.onclick = async () => {
    bottom.innerHTML = `<div class="message-locked">Loadout locked. Entering portal...</div>`;
    await enterPortal();
  };
}
...
async function init() {
  ...
  supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
    auth: { flowType: 'pkce', detectSessionInUrl: true, storage: { getItem: k=>localStorage.getItem(k), setItem:(k,v)=>localStorage.setItem(k,v), removeItem:k=>localStorage.removeItem(k) } }
  });
  try {
    await loadData();
  } catch (e) {
