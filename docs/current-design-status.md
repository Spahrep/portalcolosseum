...
- **Hit feedback (PC-DEC-048, decided by Spahrep 2026-09-18; shipped main 986cd97 as PC-70)**: monster→player hits jolt the run-window contents (`#container`; page background and status dock stay put); player→monster hits shake that monster's card + white-flash its sprite box (hit-confirmed signal). Misses/Ready/defeat get no feedback. Amplitudes/durations are gamedesigner-tuned constants (`HIT_FEEDBACK` in battle-app.js, keyframes in run.html). Fires as the feed line starts typing; intro-countdown fires shake live, the intro-snap history re-type is suppressed. Sprite change on hit (damaged sprite variant) = TODO — no sprites yet, tracked in battle-status-ui.md Open/Visual Nits.
