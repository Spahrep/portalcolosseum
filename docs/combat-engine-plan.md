...
- **Death:** all monsters dead → battle won; player HP ≤ 0 → run dead. When a monster is killed, all of its queued attacks are removed from the queue immediately (PC-DEC-054 — death-cancels-everything; a dead monster never attacks again, even on the tic it died). Player kill-cancel (PC-DEC-046): explicit-target winding attacks whose targets are all dead cancel into cooldown; auto-target attacks redirect to the first living monster. Ties: player resolves first — locked.
