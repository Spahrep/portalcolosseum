...
- **JSDoc typedefs** (TypeScript-compatible via checkJs — this is the "TypeScript interface" surface for a plain-ESM repo): `PotionSlot`, `PotionEffectType`, `PotionCategory`, `HealEffect`, `BuffEffect`, `PotionEffectPayload` (discriminated union on `type`), `PotionUseRequest` (`{slot, phase}`), `PotionTiming` (`{preTicks, postTicks, totalTicks}`), `CombatBuff` (`{name, value, endTic}`), `PotionInstanceSummary` (instance_id, template_name, effect_type, effect_label, grade, used).
