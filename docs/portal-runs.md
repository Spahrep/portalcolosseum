...
- Die selection is random (player does not choose) with a tension-building animation (e.g., roulette wheel).
  - **Animation spec (Spahrep 2026-09-15, refined same day)**: casino/roulette style — at run start and in between each battle starting, a highlight or indicator sweeps across all the dice boxes quickly, slows down, and lands on a **random** box. The row stays in normal order (Green → Yellow → Red) and the landing box is uncorrelated with the drawn die; the landed box morphs into the drawn die after the sweep stops (reveal at the roll, no spoiler during the sweep). Decided by Spahrep, 2026-09-15.
