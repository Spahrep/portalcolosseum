...
- End Run button: always visible in run.html top bar. Opens confirm dialog with exact copy: "you will lose all loot from this run and nothing will be refunded. Type 'End Run' to confirm." Confirm button disabled until input trims + lowercases to exactly 'end run'. **Enter in the input confirms, same as clicking the button** — no mouse required (PC-DEC-033, Decided by Spahrep, 2026-09-17). On confirm: POST abandon endpoint, clear localStorage currentRunId, redirect to town. Post-battle Stop run button untouched.
